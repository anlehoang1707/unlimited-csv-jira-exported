...
# Credentials Form
with st.form("Credential form"):
    ## Jira Host (type: https://your-domain.atlassian.net)
    jira_host = st.text_input(label="Your JIRA Host",value="https://leanprojectmanager.atlassian.net",disabled=False)

    api_url_v3 = f"{jira_host}/rest/api/3"

    ## JIRA account
    jira_account = st.text_input(label="Your JIRA Account",placeholder="Please enter your email",value="anle.projectmanager@gmail.com")
    ...
    connect_jira_button = st.form_submit_button(label="Connect to JIRA",type="primary")
    if connect_jira_button:
        # Send a quick request to check response status code
        response = requests.request("GET",f"{api_url_v3}/announcementBanner",headers=HEADERS,auth=AUTH)
